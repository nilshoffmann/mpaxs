...
public class MaltcmsCompletionService<T extends Serializable> implements
    ...
    private void init() {
        ...
        failed = 0;
        cancelled = 0;
        futureToTaskMap = Collections.synchronizedMap(
                new LinkedHashMap<Future<T>, Callable<T>>());
//        e = Executors.newFixedThreadPool(myMaxThreads);
    ...
    public MaltcmsCompletionService(ExecutorService es,
            ...
            boolean myBlockingWait) {
        this(es, myTimeToWaitForTasks, TimeUnit.valueOf(
                myTimeUnitToWaitForTasks), myBlockingWait);
    }
    ...
    public List<T> call() throws Exception {
        ...
        try {
            ...
            System.out.println(
                    "Cancelling " + futureToTaskMap.size() + " tasks!");
        ...
        System.out.println(
                "Retrieved all results. " + done + " jobs succeeded, " + failed + " failed, " + cancelled + " were cancelled.");
            ...
            if (callables - (failed + cancelled) > 0) {
                if (!e.awaitTermination(myTimeToWaitForTasks,
                    ...
                    if (!e.awaitTermination(myTimeToWaitForTasks,
                            myTimeUnitToWaitForTasks)) {
                        System.err.println(
                                "Thread pool did not terminate after waiting for " + myTimeToWaitForTasks + " " + myTimeUnitToWaitForTasks);
                    }
                }
            } else {
                e.shutdownNow();
    ...
    public List<Callable<T>> getFailedTasks() {
        return new ArrayList<Callable<T>>(
                futureToTaskMap.values());
    }
        ...
        if (f != null) {
            ...
            try {
                if (myBlockingWait) {
                    ...
                    System.out.println(
                            "Waiting for " + myTimeToWaitForTasks + " " + myTimeUnitToWaitForTasks + " for computation to complete");
                    try {
                        t = f.get(myTimeToWaitForTasks, myTimeUnitToWaitForTasks);
                    } catch (InterruptedException ie) {
                        System.err.println(
                                "Interrupted while waiting " + myTimeToWaitForTasks + " " + myTimeUnitToWaitForTasks + " for computation to finish!");
                        Thread.currentThread().interrupt();
                        return retrievedResult;
                    } catch (TimeoutException te) {
                        System.err.println(
                                "Timed out while waiting " + myTimeToWaitForTasks + " " + myTimeUnitToWaitForTasks + " for computation to finish!");
                        return retrievedResult;
                    }
                }
                ...
                if (t != null) {
                    ...
                    System.out.println(
                            getClass().getSimpleName() + ": " + done + " of " + callables + " submitted jobs finished. " + failed + " jobs failed, " + cancelled + " were cancelled!");
                ...
                Logger.getLogger(
                        MaltcmsResubmissionCompletionService.class.getName()).
    ...
    @Override
    public Future<T> submit(Callable<T> c) throws RejectedExecutionException, NullPointerException {
        if (!(c instanceof Serializable)) {
            throw new RejectedExecutionException(
                    "Callable must extend Serializable!");
        }
//        if (c instanceof Serializable) {
//            Callable<List<? extends T>> c1 = (Callable<List<? extends T>>) c;
        ...
        return f;
//        }
//        throw new RejectedExecutionException("Callable "+c.getClass().getName()+" does not implement Serializable!");
    }
    ...
    public Future<T> submit(Runnable r, T t) throws RejectedExecutionException, NullPointerException {
        if (!(r instanceof Serializable)) {
            throw new RejectedExecutionException(
                    "Runnable must extend Serializable!");
        }
        if (!(t instanceof Serializable)) {
            throw new RejectedExecutionException(
                    "Return type t must extend Serializable!");
        }
