    ...
    public List<T> call() throws Exception {
        ...
        try {
            ...
            if (!futureToTaskMap.keySet().isEmpty()) {
                Logger.getLogger(MaltcmsCompletionService.class.getName()).log(Level.FINEST,
            ...
            if (callables - (failed + cancelled) > 0) {
                if (!e.awaitTermination(myTimeToWaitForTasks,
                    ...
                    if (!e.awaitTermination(myTimeToWaitForTasks,
                            myTimeUnitToWaitForTasks)) {
                        Logger.getLogger(
                                MaltcmsCompletionService.class.getName()).log(
                                Level.SEVERE,
...
//                        System.out.println("Interrupted while waiting for some time!");
                        Logger.getLogger(
                                MaltcmsCompletionService.class.getName()).log(Level.WARNING, "Interrupted while waiting "
                                + myTimeToWaitForTasks + " "
                                + myTimeUnitToWaitForTasks
...
//                        System.out.println("Timeout while waiting for some time!");
                        Logger.getLogger(
                                MaltcmsCompletionService.class.getName()).log(Level.WARNING, "Timed out while waiting "
                                + myTimeToWaitForTasks + " "
                                + myTimeUnitToWaitForTasks
                                + " for computation to finish!");
                        return false;
...
//                    System.out.println("Retrieved a valid result");
                ...
                Logger.getLogger(
                        MaltcmsCompletionService.class.getName()).log(Level.WARNING,
                        "Job was cancelled: \n"
                        + ce.getLocalizedMessage());
                cancelled++;
            } catch (Exception ee) {
                Logger.getLogger(
                        MaltcmsCompletionService.class.getName()).log(Level.SEVERE, null, ee);
                failed++;
            }
        }
